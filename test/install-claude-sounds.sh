...
# 显示交互式配置菜单（完全按照 ArrowKeysMenu 的方式）
show_interactive_menu() {
    ...
    while true; do
        ...
        # 读取按键 - 完全按照 ArrowKeysMenu 的方式
        # 没有 stty，没有缓冲区清理，就是简单的 read -rsk1
        local input=""
        read -rsk1 input 2>/dev/null || input=""

        # 处理按键 - ArrowKeysMenu 的方式
        case $input in
            A)  # 上键
...
if [[ "$use_interactive" =~ ^[Yy]$ ]]; then
    if is_interactive; then
        # 清空输入缓冲区，防止之前的回车键影响交互式菜单
        # 使用带超时的循环来清空
        local dummy=""
        while read -t 0.1 -k 1 dummy 2>/dev/null; do :; done
